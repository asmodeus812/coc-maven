...
import { commands, Position, Selection, TextEdit, TextEditor, window, workspace, WorkspaceEdit } from "coc.nvim";

// tslint:disable-next-line: export-name
export async function applyWorkspaceEdit(edit: ls.WorkspaceEdit): Promise<void> {
    const workspaceEdit: WorkspaceEdit = edit;
    if (workspaceEdit !== undefined) {
        ...
        try {
            ...
            if (!currentEditor || currentEditor.document.uri !== workspaceEdit.entries()[0][0].fsPath) {
                return;
            }
...
async function executeRangeFormat(editor: TextEditor, startPosition: Position, lineOffset: number): Promise<void> {
    const endPosition: Position = editor.document.positionAt(
        editor.document.offsetAt(new Position(startPosition.line + lineOffset + 1, 0)) - 1
    );
