...
export async function getArtifacts(keywords: string[]): Promise<IArtifactMetadata[]> {
    ...
    const raw: string = await window.withProgress({ title: "Getting the artifacts list..." }, async () => {
        return httpsGet(`${URL_MAVEN_SEARCH_API}?${toQueryString(params)}`);
    });
    try {
...
export async function getVersions(gid: string, aid: string): Promise<IVersionMetadata[]> {
    ...
    const raw: string = await window.withProgress({ title: "Pulling version information..." }, async () => {
        const result = await httpsGet(`${URL_MAVEN_SEARCH_API}?${toQueryString(params)}`);
        return result;
    });
    try {
        return _.get(JSON.parse(raw), "response.docs", []);
...
export async function getLatestVersion(gid: string, aid: string): Promise<string | undefined> {
    try {
        ...
        const raw: string = await window.withProgress({ title: "Getting the latest version..." }, async () => {
            return httpsGet(`${URL_MAVEN_SEARCH_API}?${toQueryString(params)}`);
        });
