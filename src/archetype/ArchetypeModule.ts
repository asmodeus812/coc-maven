...
import { executeInBackground, getEmbeddedMavenWrapper, getMaven } from "../utils/mavenUtils";
...
export class ArchetypeModule {
    ...
    public static async listArchetypeFromXml(xmlString: string): Promise<Archetype[]> {
        try {
            ...
            const catalog: any = xmlObject["archetype-catalog"];
            const dict: { [key: string]: Archetype } = {};
            const archetypeList: any[] = catalog.archetypes[0].archetype;
            archetypeList.forEach((archetype) => {
                const groupId: string = archetype?.groupId[0];
                const artifactId: string = archetype?.artifactId[0];
                const description: string = archetype?.description[0];
                const version: string = archetype?.version[0];
                const repository: string = archetype?.repository[0];
...
async function executeInTerminalHandler(metadata: IProjectCreationMetadata): Promise<void> {
    const { archetypeArtifactId, archetypeGroupId, archetypeVersion, groupId, artifactId, targetFolder } = metadata;
    ...
    const task = async (p: coc.Progress<{ message?: string; increment?: number }>) => {
    ...
    await promptOnDidProjectCreated(archetypeArtifactId, targetFolder as string);
}
...
async function createBasicMavenProject(metadata: IProjectCreationMetadata): Promise<void> {
    ...
    const task = async (p: coc.Progress<{ message?: string; increment?: number }>) => {
        ...
        const templateUri: string = getPathToExtensionRoot("resources", "projectTemplate");
        const targetUri: string = path.join(targetFolder, artifactId);
        await fse.copy(templateUri, targetUri, { overwrite: true, errorOnExist: false });

        // update groupId/artifactId in pom.xml
        p.report({ message: "Updating pom.xml file...", increment: 10 });
        const pomUri: string = path.join(targetUri, "pom.xml");
        let pomContent = await workspace.readFile(pomUri);
        let parentPom = "";
        ...
        await fse.writeFile(pomUri, Buffer.from(pomContent));

        // create source files
        p.report({ message: "Creating source files...", increment: 20 });
        await fse.ensureDir(path.join(targetUri, "src", "main", "java"));
        await fse.ensureDir(path.join(targetUri, "src", "main", "resources"));
        await fse.ensureDir(path.join(targetUri, "src", "test", "java"));
        const packageUri: string = path.join(targetUri, "src", "main", "java", ...groupId.split("."));
        await fse.ensureDir(packageUri);
        const mainUri: string = path.join(packageUri, "Main.java");
        ...
        await fse.writeFile(mainUri, Buffer.from(content));

        // Update parent pom on demand
        if (metadata.parentProject) {
            p.report({ message: "Updating the parent pom.xml...", increment: 20 });
...
async function updateParentPom(parentPomPath: string, subModuleName: string): Promise<void> {
    if (!(await fse.pathExists(parentPomPath))) {
        throw new UserError(`Specified POM ${parentPomPath} does not exist.`);
    }

    const parentPomUri: Uri = Uri.file(parentPomPath);
    const baseDocument: coc.Document = await coc.workspace.openTextDocument(parentPomUri);
    const pomDocument: coc.TextDocument = baseDocument.textDocument;
    const documentText = pomDocument.getText();
    const xmlDocument = parseDocument(documentText);
    if (!xmlDocument) {
        return;
    }
